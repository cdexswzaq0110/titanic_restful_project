...
@app.route("/ml/<page>")
def ml_page(page):
    if page not in {"dashboard", "train", "models", "predict", "batch"}:
        return "Page not found", 404
    return render_template("ml.html", page=page)


@app.route("/api/ml/dashboard", methods=["GET"])
def ml_dashboard():
    return jsonify(ml_service.dashboard(DATABASE))
...
def ml_train():
    ...
    try:
        ml_service.start_training(
            DATABASE, MODELS_DIR, data.get("model_name", "logistic_regression"), data.get("search_mode", "quick")
        )
...
def ml_train_status():
    return jsonify(ml_service.training_status())


@app.route("/api/ml/train/result", methods=["GET"])
def ml_train_result():
    state = ml_service.training_status()
    return (jsonify(state["result"]), 200) if state["status"] == "completed" else (jsonify(state), 409)


@app.route("/api/ml/models", methods=["GET"])
def ml_models():
    return jsonify({"items": ml_service.list_models(MODELS_DIR)})


@app.route("/api/ml/models/use", methods=["POST"])
def ml_models_use():
    try:
        data = request.get_json(silent=True) or {}
        return jsonify(ml_service.activate_model(MODELS_DIR, data.get("version", "")))
...
def ml_predict():
    try:
        return jsonify(ml_service.predict(MODELS_DIR, request.get_json(silent=True) or {}))
    except (ValueError, FileNotFoundError) as error:
        return jsonify({"error": str(error)}), 400


@app.route("/api/ml/predict/what-if", methods=["POST"])
def ml_predict_what_if():
    try:
        return jsonify({"items": ml_service.what_if(MODELS_DIR, request.get_json(silent=True) or {})})
    except (ValueError, FileNotFoundError) as error:
        return jsonify({"error": str(error)}), 400
...
def ml_predict_csv():
    ...
    try:
        stream = ml_service.predict_csv(MODELS_DIR, request.files["file"])
        return send_file(stream, mimetype="text/csv", as_attachment=True, download_name="titanic_predictions.csv")
    except (ValueError, FileNotFoundError) as error:
        return jsonify({"error": str(error)}), 400
...
def ml_feature_importance():
    try:
        return jsonify(ml_service.feature_importance(MODELS_DIR))
    except FileNotFoundError as error:
        return jsonify({"error": str(error)}), 400
